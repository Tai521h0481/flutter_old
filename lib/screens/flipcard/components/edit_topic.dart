...
class _StudySetScreenState extends State<EditTopic> {
  ...
  void onSavePressed() async {
    ...
    try {
      ...
      List<dynamic> data = await LocalStorageService().getData('topics');
      await getTopicByID(_topicId, _token).then(
        (value) async => {
          updated = value['topic'],
          print('Updated topic: $updated'),
          data = await LocalStorageService().updateById(data, _topicId, updated),
          print('Updated topics: $data'),
          await LocalStorageService().saveData('topics', data),
          // await LocalStorageService().saveData('topic_$_topicId', updated),
          await getVocabularyByTopicId(_topicId, _token)
          .then((value) => {
            LocalStorageService().saveData('topic_$_topicId', value),
          }),
        },
      );
