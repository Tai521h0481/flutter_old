...
class _FlipCardScreenState extends State<FlipCardScreen> {
  ...
  void _loadInitialData() async {
    ...
    if (args is Map<String, dynamic> && args.containsKey("_id")) {
      ...
      if (storedTopics != null) {
        setState(() {
          topics = storedTopics;
          print('Loaded topics from local storage: $topics');
        });
        return;
      }
  ...
  Future<void> _loadTopics() async {
    dynamic vocab = await LocalStorageService().getData('topic_$topicId');
    if (vocab != null) {
      setState(() {
        topics = vocab;
      });
        ...
        deleteTopic(token, topicId).then((value) async {
          if ((value?['message'] ?? '') != '') {
            ...
            List<dynamic> newData =
                await LocalStorageService().deleteById(data, topicId);
            await LocalStorageService().saveData('topics', newData);
            Navigator.pushNamedAndRemoveUntil(
                context, InitScreen.routeName, (route) => false);
          } else {
            QuickAlert.show(
